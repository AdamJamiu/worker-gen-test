...
const vehicle_type = ["Type 1", "Type 2"];

const VehiclesTable = ({ onOpenModal }: IVehiclesTable) => {
  ...
  const [vehicleType, setVehicleType] = useState({ label: "", value: "" });

  // const { data: Vehicles = [], isLoading } = useQuery<IUsersListResponse[]>({
  //   queryKey: ["Vehicles-list"],
  //   queryFn: async () =>
  //     adminCaller.get("Vehicless").then((res) => res.data?.data),
  //   refetchOnWindowFocus: false,
  //   enabled: false,
  // });

  const [filteredVehicless, setFilteredVehicless] = useState<
  ...
  const handleViewVehicle = (id: string) => {
    navigate.push(`/dashboard/company-management/1/vehicles/${id}`);
    setOpenMenuId(null);
  };
  ...
  useEffect(() => {
    ...
    if (currentItems?.length > 0) {
      if (searchQuery?.length > 0) {
        ...
        setFilteredVehicless(data || []);
      } else {
        setFilteredVehicless(vehicles);
      }
    }
            ...
            <div className="w-full flex justify-start items-start gap-7 sm:gap-3 flex-wrap">
              <div className="w-[40%] sm:w-[50%] flex-1">
                <Select
                  ...
                  onChange={(e) =>
                    setVehicleType({ label: e.label, value: e.value })
                  }
          ...
          <table className="w-full min-w-max whitespace-nowrap overflow-hidden px-4 md:px-5 py-4 bg-white rounded-lg">
            <thead className="text-neutrals800 text-xs sm:text-sm md:text-base font-bricolage border-b border-neutrals100 rounded-t-lg">
              <tr className="w-full bg-primary100">
                <th className="p-3 rounded-tl-xl font-medium">
                  <div
                    title="Comapny name"
                ...
                <th className="p-3 font-medium">Assigned driver</th>
                <th className="p-3 font-medium">Vehicle status</th>
                <th className="px-4 py-2 rounded-tr-xl font-medium">Action</th>
              </tr>
            </thead>
            <tbody>
              {filteredVehicless?.map((item, index) => (
                <tr
                  key={index}
